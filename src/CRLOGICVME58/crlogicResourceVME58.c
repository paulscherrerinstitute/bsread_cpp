...
static void initializeVME58 (resource *self, rstatus status, char* emessage) {
...
static void readVME58(resource *self, double *message){
...
static void initResourceVME58 (resource *self) {
    self->initialize = (void *) &initializeVME58;
    self->read = (void *) &readVME58;
}
 ...
 */
rstatus crlogicAddVME58MotorResource(char* key, int baseAddress, int channel){
	resourceListItem* newNode;

	printf("Add VME58 motor resource\n");

	/* Append at the beginning */
	newNode = calloc (1, sizeof(resourceListItem) );

	initResourceVME58( &(newNode->res));
